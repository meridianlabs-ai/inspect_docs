---
title: Articles
llms-description: Writing prose pages alongside your API reference—navigation, diagrams, and README handling.
---
...
Add more articles by creating `.qmd` file anywhere in your docs directory with a title and (optionally) a description in the frontmatter:
...
Quarto already has support for [Mermaid](https://quarto.org/docs/authoring/diagrams.html#mermaid) and [Graphviz](https://quarto.org/docs/authoring/diagrams.html#graphviz) diagrams—both are good choices for flowcharts, sequence diagrams, and other graph-style visuals written in code. See [Quarto's diagram documentation](https://quarto.org/docs/authoring/diagrams.html) for the full set of options.

In addition to these, Inspect Docs adds support for [Excalidraw](https://excalidraw.com)—useful when you want something hand-drawn or freeform rather than code-generated. You embed an Excalidraw diagram by referencing a `.excalidraw` file as an image; the extension converts it to SVG at build time (Node.js dependencies are installed on first use and results are cached):
