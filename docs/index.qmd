...
When `external_refs` is configured, inline code also resolves against other Inspect Docs sites. See [Interlinks](interlinks.qmd) to learn more.

## llms.txt

Every site includes an auto-generated [`llms.txt`](https://llmstxt.org) at the root and per-page Markdown sources at `{page}.html.md`. A **Copy page** button in the page header lets readers grab the Markdown or open it in a new tab. See [llms.txt](llms.qmd) for details.
