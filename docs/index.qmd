---
title: Inspect Docs
llms-description: A Quarto extension for Python package documentation with automatic API reference generation.
---
...
- Automatically turn inline code references (e.g. ``` `my_function()` ```) into links to the right reference page.
...
When `external_refs` is configured, inline code also resolves against other Inspect Docs sites — and any site built with this extension is automatically a valid cross-link target for others. See [Interlinks](interlinks.qmd) to learn more.

## llms.txt

Every site ships with an auto-generated [`llms.txt`](https://llmstxt.org) at the root and per-page Markdown sources (served at `{page}.html.md`), so LLMs can ingest your docs directly. 

A **Copy page** button in the page header lets readers grab the Markdown or open it in a new tab — handy for pasting into a chat. See [llms.txt](llms.qmd) for full details including how to customize page descriptions for the main index.
